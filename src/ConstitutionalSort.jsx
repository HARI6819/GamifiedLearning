...
import sortQuestionsLocal from "./data/sortQuestions.json";
...
export default function ConstitutionalSort() {
    ...
    const { data: sortQuestions } = useGameQuestions("sort", sortQuestionsLocal, false);
    ...
    const startGame = () => {
        ...
        let difficultyItems = sortQuestions[difficulty];
    ...
    const categories = allActiveItems.length > 0
        ...
        : Array.from(new Set(sortQuestions[difficulty].map(i => i.category)));
        ...
        <>
            ...
            <main className="sort-container">
                <header className="sort-header">
                    ...
                    <div>
                        <h1>⫽ <TranslatedText>{t.constitutionalSort.title}</TranslatedText></h1>
                        <p><TranslatedText>{t.constitutionalSort.desc}</TranslatedText></p>
                    </div>
                </header>
                ...
                {gameState === "start" && (
                    <div className="start-screen">
                        ...
                        <h2><TranslatedText>{t.constitutionalSort.title}</TranslatedText></h2>
                        <p style={{ fontFamily: "sans-serif", marginBottom: "20px", fontSize: ".9rem", maxWidth: "80%", color: "grey" }}>
                            <TranslatedText>{t.constitutionalSort.desc}</TranslatedText>
                        </p>
                        <h4><TranslatedText>{t.constitutionalSort.difficulty}</TranslatedText></h4>
                        ...
                        <button className="start-btn" onClick={startGame}>
                            <TranslatedText>{t.constitutionalSort.startGame}</TranslatedText>
                        </button>
                    </div>
                )}
                    ...
                    <div className="Container-box-sort">
                        <div className="game-info-bar">
                            ...
                            <div className="info-item">
                                <TranslatedText>{t.constitutionalSort.score}</TranslatedText>: {score}
                            </div>
                            <button className="restart-btn" onClick={() => setGameState("start")}>
                                <RotateCcw size={16} /> <TranslatedText>{t.constitutionalSort.restart}</TranslatedText>
                            </button>
                        </div>
                            ...
                            >
                                ...
                                {items.length === 0 && (
                                    <button className="submit-game-btn animated pulse" onClick={handleSubmit}>
                                        <CheckCircle2 size={20} /> <TranslatedText>{t.constitutionalSort.submit}</TranslatedText>
                                    </button>
                                )}
                            </aside>
                            ...
                            <section className="categories-grid">
                                {categories.map((cat) => (
                                    <div
                                        ...
                                        onTouchEnd={handleTouchEnd}
                                    >
                                        <h3><TranslatedText>{t.constitutionalSort.categoryLabels[cat] || cat}</TranslatedText></h3>
                ...
                {gameState === "won" && (
                    <div className="win-overlay animated fadeIn">
                        <div className="win-card">
                            ...
                            <h2><TranslatedText>{t.constitutionalSort.wellDone}</TranslatedText></h2>
                            <p><TranslatedText>{t.constitutionalSort.wellDoneDesc}</TranslatedText></p>
                            <div className="win-stats">
                                <div className="stat-pill">
                                    <Timer size={16} /> {(difficulty === "Hard" ? 45 : difficulty === "Medium" ? 60 : 90) - timer}s <TranslatedText>{t.constitutionalSort.used}</TranslatedText>
                                </div>
                                ...
                                <div className="stat-pill">
                                    <TranslatedText>{t.constitutionalSort.points}</TranslatedText>: {score}
                                </div>
                            </div>
                            <button className="play-again-btn" onClick={() => setGameState("start")}>
                                <TranslatedText>{t.constitutionalSort.playAgain}</TranslatedText>
                            </button>
                        </div>
                    </div>
                )}
                ...
                {gameState === "lost" && (
                    <div className="win-overlay animated fadeIn">
                        <div className="win-card">
                            ...
                            <h2><TranslatedText>{t.constitutionalSort.gameOver}</TranslatedText></h2>
                            <p><TranslatedText>{t.constitutionalSort.gameOverDesc}</TranslatedText></p>
                            <div className="win-stats">
                                <div className="stat-pill">
                                    <TranslatedText>{t.constitutionalSort.remaining}</TranslatedText>: {items.length} <TranslatedText>{t.constitutionalSort.items}</TranslatedText>
                                </div>
                            </div>
                ...
                {touchItem && (
                    ...
                    >
                        <Sparkles size={16} />
                        <TranslatedText>{touchItem.text}</TranslatedText>
                    </div>
                )}
            </main>
            <Footer />
