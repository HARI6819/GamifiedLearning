...
import crossroadsQuestionsLocal from "./data/crossroadsQuestions.json";
...
export default function ConstitutionalCrossroads() {
    ...
    const { data: crossroadsQuestions } = useGameQuestions("crossroads", crossroadsQuestionsLocal, false);
    ...
    const startGame = () => {
        ...
        let scenarioList = crossroadsQuestions[difficulty];
        ...
        <>
            ...
            <main className={`crossroads-container ${theme}`}>
                <header className="crossroads-header">
                    ...
                    <div>
                        <h1>⚖️<TranslatedText>{t.constitutionalCrossroads.title}</TranslatedText></h1>
                        <p><TranslatedText>{t.constitutionalCrossroads.desc}</TranslatedText></p>
                    </div>
                </header>
                ...
                {gameState === "start" && (
                    <div className="start-screen">
                        ...
                        <h2><TranslatedText>{t.constitutionalCrossroads.startTitle}</TranslatedText></h2>
                        <p style={{ fontFamily: "sans-serif", marginBottom: "20px", fontSize: ".9rem", maxWidth: "80%", color: "grey" }}>
                            <TranslatedText>{t.constitutionalCrossroads.startDesc}</TranslatedText>
                        </p>
                        <h4><TranslatedText>{t.constitutionalCrossroads.difficulty}</TranslatedText></h4>
                        ...
                        <button className="start-btn" onClick={startGame}>
                            <TranslatedText>{t.constitutionalCrossroads.startGame}</TranslatedText>
                        </button>
                    </div>
                )}
                ...
                {gameState === "scenario" && currentScenario && (
                    <div className="scenario-screen">
                        <div className="scenario-progress">
                            <span><TranslatedText>{t.constitutionalCrossroads.scenario}</TranslatedText> {currentScenarioIndex + 1}/{scenarios.length}</span>
                            <span><TranslatedText>{t.constitutionalCrossroads.score}</TranslatedText>: {score}</span>
                        </div>

                        <div className="scenario-card">
                            <div className="scenario-title">
                                <Scale size={24} />
                                <h3><TranslatedText>{currentScenario.title}</TranslatedText></h3>
                            </div>
                            <div className="scenario-situation">
                                <p><TranslatedText>{currentScenario.situation}</TranslatedText></p>
                            </div>
                            ...
                            <div className="choices-container">
                                {currentScenario.choices.map((choice) => (
                                    ...
                                    >
                                        ...
                                        <span className="choice-text"><TranslatedText>{choice.text}</TranslatedText></span>
                                    </button>
                                ))}
                            </div>
                            ...
                            >
                                <TranslatedText>Submit Answer</TranslatedText>
                            </button>
                        </div>
                    </div>
                )}
                        ...
                        <div className="feedback-card">
                            <div className={`feedback-header ${isCorrect ? 'correct' : 'incorrect'}`}>
                                {isCorrect ? (
                                    <>
                                        <CheckCircle size={48} />
                                        <h2><TranslatedText>{t.constitutionalCrossroads.correct}</TranslatedText></h2>
                                    </>
                                ) : (
                                    <>
                                        <XCircle size={48} />
                                        <h2><TranslatedText>{t.constitutionalCrossroads.incorrect}</TranslatedText></h2>
                                    </>
                                )}
                            </div>
                            ...
                            <div className="feedback-content">
                                <div className="correct-answer-section">
                                    <strong><TranslatedText>Correct Answer:</TranslatedText></strong>
                                    <p><TranslatedText>{currentScenario.choices.find(c => c.id === currentScenario.correctChoice)?.text}</TranslatedText></p>
                                </div>

                                <div className="explanation-section">
                                    <div className="section-header">
                                        <BookOpen size={20} />
                                        <strong><TranslatedText>{t.constitutionalCrossroads.explanation}</TranslatedText></strong>
                                    </div>
                                    <p><TranslatedText>{currentScenario.explanation}</TranslatedText></p>
                                </div>

                                <div className="articles-section">
                                    <strong><TranslatedText>{t.constitutionalCrossroads.relevantArticles}</TranslatedText>:</strong>
                ...
                {gameState === "complete" && (
                    <div className="crossroads-completion-overlay animated fadeIn">
                        <div className="crossroads-completion-card">
                            ...
                            <h2><TranslatedText>{t.constitutionalCrossroads.completion.title}</TranslatedText></h2>
                            <p><TranslatedText>{t.constitutionalCrossroads.completion.desc}</TranslatedText></p>

                            <div className="completion-stats">
                                <div className="stat-pill">
                                    <TranslatedText>{t.constitutionalCrossroads.completion.correctAnswers}</TranslatedText>: {correctAnswers}/{scenarios.length}
                                </div>
                                <div className="stat-pill">
                                    <TranslatedText>{t.constitutionalCrossroads.completion.totalScore}</TranslatedText>: {score}
                                </div>
                            </div>

                            <button className="continue-btn-crossroads" onClick={() => setGameState("start")}>
                                <RotateCcw size={18} /> <TranslatedText>{t.constitutionalCrossroads.completion.playAgain}</TranslatedText>
                            </button>
                        </div>
                    </div>
                )}
