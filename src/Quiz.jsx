...
import "./Quiz.css";
import quizDataLocal from "./data/quizData.json";
...
export default function Quiz() {
    ...
    const { data: quizDataDB } = useGameQuestions("quiz", quizDataLocal, false);
    ...
    const startQuiz = () => {
        ...
        let localizedQuestions = quizDataDB[difficulty] || [];
        ...
        <>
            ...
            <main className="quiz-container">
                ...
                <header className="quiz-header">
                    ...
                    <div>
                        <h1>🧠<TranslatedText>{t.quiz.title}</TranslatedText></h1>
                        <p><TranslatedText>{t.quiz.desc}</TranslatedText></p>
                    </div>
                </header>
                ...
                {gameState === "difficulty" && (
                    <div className="start-screen-quiz">
                        ...
                        <h2><TranslatedText>{t.quiz.title}</TranslatedText></h2>
                        <p><TranslatedText>{t.quiz.startDesc}</TranslatedText></p>

                        <h4 style={{ fontFamily: "'Times New Roman', serif", fontWeight: 700, marginTop: '1rem' }}><TranslatedText>{t.quiz.difficulty}</TranslatedText></h4>
                        ...
                        <button className="start-btn-quiz" onClick={startQuiz}>
                            <TranslatedText>{t.quiz.startBtn}</TranslatedText>
                        </button>
                    </div>
                )}
                    ...
                    <div className="playing-screen-quiz">
                        <div className="game-info-bar-quiz">
                            <div className="quiz-info">
                                <span style={{ fontWeight: 600 }}><TranslatedText>{t.quiz.questionCount}</TranslatedText> {currentIdx + 1} / 10</span>
                                <span style={{ marginLeft: '12px', fontSize: '0.8rem', padding: '4px 10px', background: 'rgba(59, 130, 246, 0.1)', borderRadius: '20px', color: '#3b82f6' }}><TranslatedText>{t.common.difficulty[difficulty]}</TranslatedText></span>
                            </div>
                        ...
                        <div className="question-card-quiz">
                            <h3><TranslatedText>{questions[currentIdx]?.question}</TranslatedText></h3>
                                ...
                                {questions[currentIdx]?.options.map((opt, idx) => {
                                    ...
                                    return (
                                        ...
                                        >
                                            ...
                                            <span><TranslatedText>{opt}</TranslatedText></span>
                ...
                {gameState === "finished" && (
                    <div className="quiz-completion-overlay animated fadeIn">
                        <div className="quiz-completion-card">
                            ...
                            <h2><TranslatedText>{t.quiz.completion.title}</TranslatedText></h2>
                            <p>
                                <TranslatedText>{t.quiz.completion.desc.replace("{difficulty}", t.common.difficulty[difficulty])}</TranslatedText>
                            </p>
                            ...
                            <div className="completion-stats">
                                <div className="stat-pill">
                                    <TranslatedText>{t.quiz.completion.points}</TranslatedText>: {score * (difficulty === "Hard" ? 10 : difficulty === "Medium" ? 7 : 5)}
                                </div>
                                <div className="stat-pill">
                                    <TranslatedText>{t.quiz.completion.diff}</TranslatedText>: <TranslatedText>{t.common.difficulty[difficulty]}</TranslatedText>
                                </div>
                            </div>
                            <div className="result-actions-quiz">
                                <button onClick={resetQuiz} className="continue-btn-quiz">
                                    <RotateCcw size={18} /> <TranslatedText>{t.quiz.completion.playAgain}</TranslatedText>
                                </button>
                            </div>
                        </div>
                    </div>
