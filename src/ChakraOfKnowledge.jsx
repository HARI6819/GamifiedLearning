...
import chakraQuestionsLocal from "./data/chakraQuestions.json";
...
export default function ChakraOfKnowledge() {
  ...
  const { data: chakraQuestions, loading: questionsLoading } = useGameQuestions("chakra", chakraQuestionsLocal, true);
  ...
  const spinWheel = () => {
    ...
    setTimeout(() => {
      ...
      const randomQuestion = qPool.length > 0 ? qPool[Math.floor(Math.random() * qPool.length)] : null;
      if (!randomQuestion) {
        setIsSpinning(false);
        return;
      }
    ...
    <>
      ...
      <main className="chakra-page">
        {articlesRead < 10 && !loading && (
          <div className="locked-overlay-chakra">
            <div className="locked-card-chakra">
              ...
              <h2><TranslatedText>{t.gamesPage?.lockedMessage || "Game Locked!"}</TranslatedText></h2>
              <p><TranslatedText>Read at least 10 articles to unlock the Chakra of Knowledge.</TranslatedText></p>
              <div className="unlock-progress-chakra">
                <div className="progress-text-chakra">
                  <span>{articlesRead} / 10 <TranslatedText>Articles Read</TranslatedText></span>
                </div>
              ...
              >
                <TranslatedText>Go to Learn</TranslatedText>
              </button>
            </div>
          </div>
        )}
        ...
        <header className="chakra-header">
          ...
          <div>
            <h1><TranslatedText>{t.chakra.title}</TranslatedText></h1>
            <p><TranslatedText>{t.chakra.desc}</TranslatedText></p>
          </div>
        </header>
          ...
          <div className="wheel-section">
            ...
            <div className="wheel-controls">
              <button
                ...
                disabled={isSpinning}
              >
                {isSpinning ? "..." : <TranslatedText>{t.chakra.spinBtn}</TranslatedText>}
              </button>
            </div>
          </div>
          ...
          <div className="quiz-section-chakra" ref={quizRef}>
            {!currentQuestion ? (
              <div className="info-card-chakra center">
                ...
                <h3 className="h3-chakra"><TranslatedText>{t.chakra.infoTitle}</TranslatedText></h3>
                <p className="p-chakra"><TranslatedText>{t.chakra.infoDesc}</TranslatedText></p>
              </div>
            ) : isLevelComplete ? (
              <div className="chakra-completion-card animated fadeIn">
                <div className="completion-icon">🏆</div>
                <h2><TranslatedText>Level Completed!</TranslatedText></h2>
                <p><TranslatedText>You've successfully answered 10 questions in</TranslatedText> <TranslatedText>{t.common.difficulty[difficulty]}</TranslatedText> <TranslatedText>mode.</TranslatedText></p>
                <div className="completion-stats">
                  <span>Questions: 10/10</span>
                  <span>Difficulty: {difficulty}</span>
                </div>
                <button className="continue-btn-chakra" onClick={resetLevel}>
                  <TranslatedText>Play Again</TranslatedText>
                </button>
              </div>
            ) : (
              <div className="chakra-quiz-card-inline">
                ...
                <div className="chakra-progress-wrap">
                  <div className="chakra-progress-text">
                    <TranslatedText>Question</TranslatedText> {answeredCount} / 10
                  </div>
                ...
                <div className="quiz-header-chakra">
                  ...
                  <span className="difficulty-tag-chakra">
                    <TranslatedText>{t.common.difficulty[difficulty]}</TranslatedText>
                  </span>
                </div>

                <h2 className="chakra-q-title"><TranslatedText>{currentQuestion.question}</TranslatedText></h2>
                ...
                {isAnswered && selectedArticle && (
                  <div className="article-fact-preview animated fadeIn">
                    <div className="article-preview-label"><TranslatedText>Did you know?</TranslatedText> (<TranslatedText>Art.</TranslatedText> {selectedArticle.number})</div>
                    <p><TranslatedText>{selectedArticle.simplifiedText}</TranslatedText></p>
                  </div>
                )}
              </div>
            )}
