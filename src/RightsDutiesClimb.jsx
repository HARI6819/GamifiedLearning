...
import questionsData from "./data/arti.json";
...
export default function RightsDutiesClimb() {
  ...
  const { data: questions } = useGameQuestions("climb", questionsData, true);
    ...
    <>
      ...
      <main className="game-container">
        <header className="game-header">
          ...
          <div>
            <h1><TranslatedText>{t.climb.title}</TranslatedText></h1>
            <p><TranslatedText>{t.climb.desc}</TranslatedText></p>
          </div>
        </header>
          ...
          <aside className="panel">
            ...
            <div className="stats">
              <div>
                ...
                <span><TranslatedText>{t.climb.position}</TranslatedText></span>
              </div>
              <div>
                <h2 style={{ color: " #EE860E" }}>{moves}</h2>
                <span><TranslatedText>{t.climb.score}</TranslatedText></span>
              </div>
              <div>
                <h2 style={{ color: " #51AA28" }}>{dice}</h2>
                <span><TranslatedText>{t.climb.dice}</TranslatedText></span>
              </div>
            </div>
            ...
            {!showQuestion && <div className="dice-box">
              ...
              <button onClick={rollDice} disabled={showQuestion || IsActiveDice} className={`roll-btn ${IsActiveDice ? "rolling" : ""}`}>
                <Dices /> <h3>{IsActiveDice ? <TranslatedText>Rolling...</TranslatedText> : <TranslatedText>{t.climb.roll}</TranslatedText>}</h3>
              </button>
            </div>}

            {showQuestion && currentQ && (
              <div className="question-box">
                <h3><TranslatedText>{currentQ.question}</TranslatedText></h3>
                {currentQ.options.map((opt, i) => (
                  ...
                  >
                    <div className="Qoptions"><h4>{String.fromCharCode(i + 65)}.</h4><TranslatedText>{opt}</TranslatedText></div>
                  </button>
                ))}
              </div>
            )}

            {message && <div className="messageBox"><TranslatedText>{message}</TranslatedText></div>}

            <button className="reset" onClick={resetGame}>
              <div style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: "5px", fontSize: "15px" }}><RotateCcw /> <TranslatedText>{t.climb.reset}</TranslatedText></div>
            </button>

            <div className="rules">
              <h4><TranslatedText>{t.climb.howToPlay}</TranslatedText></h4>
              <ul>
                {t.climb.rules.map((rule, idx) => (
                  <li key={idx}><TranslatedText>{rule}</TranslatedText></li>
                ))}
              </ul>
            </div>
          </aside>
          ...
          <div className={`climb-popup-overlay animated fadeIn`}>
            {popup.type === 'win' ? (
              <div className="climb-completion-card">
                ...
                <h2><TranslatedText>{language === 'hi' ? 'बधाई हो!' : 'Congratulations!'}</TranslatedText></h2>
                <p>
                  <TranslatedText>
                    {language === 'hi'
                ...
                <div className="completion-stats">
                  <div className="stat-pill">
                    <TranslatedText>Position</TranslatedText>: 100
                  </div>
                  <div className="stat-pill">
              ...
              <div className={`climb-popup ${popup.type}`}>
                <h2><TranslatedText>{popup.title}</TranslatedText></h2>
                <p><TranslatedText>{popup.message}</TranslatedText></p>
