    ...
    return (
      <>
        ...
        <div className="loading-screen">
          ...
          <p><TranslatedText>{t.gamesPage.progression.loading}</TranslatedText></p>
        </div>
      </>
    );
  }
        ...
        {localStorage.getItem('isGuest') === 'true' && (
          <div className="guest-restriction-overlay animated fadeIn">
            <div className="glossy-card-guest">
              ...
              <p><TranslatedText>{t.login.guestRestrictedMsg || "Please login to access the full game experience and track your progress."}</TranslatedText></p>
        ...
        {articlesRead < 10 && (
          <div className="locked-overlay">
            <div className="locked-card" ref={lockRef}>
              ...
              <h2><TranslatedText>{t.gamesPage.lockedMessage}</TranslatedText></h2>
              <div className="unlock-progress">
                <div className="progress-text">
                  <span>{articlesRead} / 10 <TranslatedText>{t.gamesPage.articlesToUnlock}</TranslatedText></span>
                </div>
              ...
              <Link to="/learn" className="unlock-btn">
                <TranslatedText>{t.home.exploreArticles}</TranslatedText>
              </Link>
            </div>
          </div>
        )}
        <div className="container1">
          <div className="header1 reveal-up stagger-1">
            <span className="pill1">
              <Gamepad2 size={14} />
              <TranslatedText>{t.gamesPage.hub}</TranslatedText>
            </span>
            <h1><TranslatedText>{t.gamesPage.title}</TranslatedText></h1>
            <p><TranslatedText>{t.gamesPage.subtitle}</TranslatedText></p>
            <div className="progression-status">
              <div className="current-tier">
                <span><TranslatedText>{t.gamesPage.progression.currentTier}</TranslatedText>:  {isHardDone ? <span>🏆<TranslatedText>Completed!</TranslatedText></span> : <strong><TranslatedText>{t.common.difficulty[globalLevel]}</TranslatedText></strong>}</span>
                {!isMediumDone && (
                  <span className="unlock-hint">
                    <TranslatedText>{globalLevel === "Easy" ? t.gamesPage.progression.unlockedMedium : t.gamesPage.progression.unlockedHard}</TranslatedText>
                  </span>
                )}
              </div>
              ...
              <div className="tier-progress">
                {globalLevel === "Easy" && !isMediumDone && (
                  <div className="progress-section">
                    <div className="progress-label">
                      <span>📈 <TranslatedText>Progress to Medium Tier</TranslatedText></span>
                      <span className="progress-count">
                        {primaryGames.filter(g => completedLevels[g]?.includes("Easy")).length} / {primaryGames.length} <TranslatedText>games</TranslatedText>
                      </span>
                    </div>
                          ...
                          >
                            ...
                            {hoveredGame === g && (
                              <div className="game-dot-tooltip">
                                <TranslatedText>{gameTitle}</TranslatedText>
                              </div>
                            )}
                ...
                {globalLevel === "Medium" && !isMediumDone && (
                  <div className="progress-section">
                    <div className="progress-label">
                      <span>📈 <TranslatedText>Progress to Hard Tier</TranslatedText></span>
                      <span className="progress-count">
                        {primaryGames.filter(g => completedLevels[g]?.includes("Medium")).length} / {primaryGames.length} <TranslatedText>games</TranslatedText>
                      </span>
                    </div>
                    <div className="progress-bar">
                      <div
                ...
                {globalLevel === "Hard" && (
                  <div className="progress-section completed-all">
                    {isHardDone ? <span>🏆 <TranslatedText>All Games Completed!</TranslatedText></span> : <span>🏆 <TranslatedText>You are in last tier!</TranslatedText></span>}
                  </div>
                )}
              </div>
            </div>
            ...
            <div className="category-filter-bar">
              {["All", "Legislature", "Executive", "Judiciary"].map((cat) => (
                ...
                >
                  <TranslatedText>{cat === "All" ? t.learn.filters.all : (cat === "Legislature" ? t.home.pillars.legislature.title : (cat === "Executive" ? t.home.pillars.executive.title : t.home.pillars.judiciary.title))}</TranslatedText>
                </button>
              ))}
            </div>
          </div>
              ...
              return (
                <div className={`game-card1 reveal-up ${staggerClass}`} key={game.id}>
                  ...
                  <div className="card-body1">
                    ...
                    <h3><TranslatedText>{game.title}</TranslatedText></h3>
                    <p className="desc1"><TranslatedText>{game.desc}</TranslatedText></p>
                  </div>
                  <div className="card-footer1">
                    <span className="time1">
                      <Clock size={14} /> <TranslatedText>{game.time}</TranslatedText>
                    </span>
                    <Link to={`${game.link}${selectedCategory !== "All" ? `?category=${selectedCategory}` : ""}`} className="play-btn1">
                      <TranslatedText>{t.home.gameFormats.playNow}</TranslatedText> <ArrowRight size={16} />
                    </Link>
                  </div>
                </div>
              );
          ...
          <div className="coming-soon1">
            ...
            <h3><TranslatedText>{t.gamesPage.comingSoon.title}</TranslatedText></h3>
            <p>
              <TranslatedText>{t.gamesPage.comingSoon.desc}</TranslatedText>
            </p>
          </div>
